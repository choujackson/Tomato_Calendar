...
export const LoginScreen: React.FC<LoginScreenProps> = ({ navigation }) => {
  ...
  const [isSplashActive, setIsSplashActive] = useState(!hasShownSplash);

  const iconSize = Math.round(Math.max(72, Math.min(100, height * 0.14)));

  const contentOpacity = useRef(new Animated.Value(isSplashActive ? 0 : 1)).current;
  const contentTranslateY = useRef(
    new Animated.Value(isSplashActive ? 12 : 0)
  ).current;
  const overlayOpacity = useRef(new Animated.Value(isSplashActive ? 1 : 0)).current;
  ...
  return (
    ...
    >
      <Animated.View
        ...
        pointerEvents={isSplashActive ? 'none' : 'auto'}
      >
        <View style={styles.scrollContent}>
          <View style={styles.header}>
            <Text style={[styles.title, { color: titleColor }]}>Tomato Calendar</Text>
            <Text style={[styles.subtitle, { color: subtitleColor }]}>Sign in to your account</Text>
          </View>
      ...
      {isSplashActive && (
        <>
          ...
          <Animated.View
            pointerEvents="none"
            style={[styles.logoWrapper, { opacity: overlayOpacity }]}
          >
